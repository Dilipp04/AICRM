...
const getClient = () => {
  ...
  if (!apiKey) {
    throw new ApiError(
      503,
      "Gemini Api key is not configure , Add api key to the .env file",
    );
  }
...
export const generateLeadSummary = async (lead) => {
  const prompt = `You are an expert B2B sales analyst for a CRM called AICRM. Analyze the following sales lead and produce a concise assessment.
  ...
  -Pontential deal Value:${lead.value || 0}
  ...
  const schema = {
    ...
    properties: {
      ...
      riskScore: {
        type: "integer",
        description: "Risk of looasing rhis deal, 0 (safe) to 100 (high risk)",
      },
      suggestedPriority: {
        type: "string",
...
export const generateEmail = async ({ lead, purpose, tone, sender }) => {
  const prompt = `You are as senior sales rep writing on behalf of ${sender?.name || "out team"} ${sender?.company ? `at ${sender.company}` : ""}.

Write a professional sales email.
Purpose: ${purpose || "follow-up"}
Desire tone : ${tone || "friendly and professional"}

Receipent (lead) details:
- Name : ${lead?.name || "there"}
- Company : ${lead?.company || "N/A"}
- Pipeline stage : ${lead?.name || "New"}
- Context / notes : ${lead?.name || "None"}
